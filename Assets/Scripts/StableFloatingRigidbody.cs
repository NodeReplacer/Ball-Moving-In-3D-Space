//CustomGravityRigidybody but for larger objects that should have more even buoyancy.
//SUMMARIZED: Each point gets its own buoyancy now to evenly float.
using UnityEngine;
...
public class StableFloatingRigidbody : MonoBehaviour {
	...
	[SerializeField]
	bool safeFloating = false; //When each point has its own buoyancy if a point gets too high up
	//it will think it is fully submerged when it definitely isn't.
	...
	void Awake () {
		...
		submergence = new float[buoyancyOffsets.Length]; //This is a big difference.
		//An array of buoyancy offsets means we can distribute the buoyancy effect over a large
